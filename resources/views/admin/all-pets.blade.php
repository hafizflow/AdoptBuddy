<x-layout>
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($pets as $pet)
                <a href="{{ route('admin.pets.edit', $pet->id) }}" class="block hover:shadow-lg transition-shadow duration-200">
                    <div class="bg-white rounded-xl shadow p-4">
                        ...
                        <p class="text-sm text-gray-600">{{ $pet->breed }} • {{ $pet->age }} years old</p>
                        <p class="text-sm mt-1">{{ $pet->description }}</p>
                        <span class="inline-block mt-2 px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">{{ ucfirst($pet->gender) }}</span>
                    </div>
                </a>
            @endforeach
        </div>
